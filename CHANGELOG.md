...
- Observatory hardening: event-chain appends now take an inter-process file lock, so a CLI command and the live daemon settling a due wait can no longer interleave sequence numbers and break the hash chain; `observatory.json` records and validates its project root, so a copied project directory never reports another project's daemon; a `serve` that loses the stable port to a concurrent spawn for the same run defers to the winner instead of double-serving on an ephemeral port.

- Always-on live Observatory: the Arena page is now guaranteed, not agent-dependent. Every engine operation refreshes a stable, bookmarkable entry page `<root>/.sisyfus/observatory.html` — a self-contained copy of the Arena that shows the state as of the last event and auto-redirects to the live daemon the moment one answers. Every `sisyfus research` command self-heals the live monitor: a dead daemon is respawned detached (`research serve`) on a stable per-project port derived from the root path, one serving a different run is retargeted, and coordinates persist in `<root>/.sisyfus/observatory.json` (liveness is always probed, never trusted from the file). Command JSON summaries now carry `observatory_entry` and, when a daemon is alive, `observatory_url`. A bare `serve` is idempotent (prints the existing URL and exits); `--port` overrides the stable port. `SISYFUS_AUTO_SERVE=0` disables respawning for CI/tests.
